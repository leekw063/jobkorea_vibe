...
- **Pass_R_No 기반 중복 체크**로 **데이터 정합성 보장**
- **순차 처리**로 안정적인 이력서 수집
- **공고 목록 탭**으로 수집된 공고 정보 즉시 확인
- **AI 이력서 검토**로 1차 스크리닝 시간 **80% 단축**
- 공고 Markdown과 이력서 Markdown을 **정밀 비교**해 AI 평가 신뢰도 향상
...
| 중복 수집 방지 | 이미 수집된 이력서는 자동으로 제외 | 중복률 0% | ✅ 완료 (Pass_R_No) |
| 공고 중복 방지 | 이미 수집된 공고는 제외 | 중복률 0% | ✅ 완료 (공고번호) |
| 안정적인 수집 | 순차 처리로 안정성 향상 | 수집 성공률 95% 이상 | ✅ 완료 |
...
| 공고 관리 기능 | 공고 목록 탭 및 상세보기 | 공고 조회 100% | ✅ 완료 |
...
2. 시스템이 `job_postings.job_detail_md`와 `resumes.md_url`의 Markdown을 로드
3. Gemini 2.0 Flash가 두 문서를 비교 분석하고 일치/불일치 근거를 인용
...
| 접수된 이력서 탐색 | 각 공고별 접수된 이력서 목록 순회 (100개씩) | Playwright | ✅ 완료 |
| 이력서 중복 체크 | Pass_R_No 기반 중복 제외 | Node.js + Supabase | ✅ 완료 |
| 순차 처리 | 이력서를 순차적으로 처리 (500ms 딜레이) | Playwright | ✅ 완료 |
| 상세 페이지 파싱 | 이름, 연락처, 학력, 경력, 자기소개서 등 추출 | Playwright | ✅ 완료 |
| 전화번호 추출 | 010 시작 번호만 추출 | Playwright | ✅ 완료 |
| 이메일 추출 | mailto 링크 기반 추출 | Playwright | ✅ 완료 |
...
| 이력서 Markdown 보관 | `resumes.md_url`에 Markdown 파일 경로 저장 | 파일 시스템 + Supabase | ✅ 완료 |
...
| 공고-이력서 매칭 분석 | 공고 Markdown (`job_detail_md`)과 이력서 Markdown (`md_url`) 비교 | ✅ 완료 |
| 다중 기준 평가 | 기술스택, 경력, 학력, 프로젝트, 커뮤니케이션 | ✅ 완료 |
| 점수 산출 | 0-100점 정수 | ✅ 완료 |
| 상세 평가 생성 | 약 1000자 분량 평가 텍스트 | ✅ 완료 |
| 결과 저장 | DB에 저장하여 재사용 | ✅ 완료 |
| Markdown 미존재 시 | 기본 메타 정보를 활용해 최소 평가 진행 | ✅ 완료 |

### 4.4. 대시보드
| 기능 | 설명 | 구현 상태 |
|------|------|----------|
| 공고 목록 탭 | 수집된 공고 목록 조회 | ✅ 완료 |
| 공고 상세보기 | 공고 Markdown 모달 표시 | ✅ 완료 |
| 이력서 목록 표시 | 지원자별 카드형/테이블형 UI | ✅ 완료 |
| 뷰 전환 | 카드 ↔ 테이블 토글 | ✅ 완료 |
| 필터/검색 기능 | 공고명, 공고번호, 상태, 지원일자 기준 필터 | ✅ 완료 |
| 상태 변경 | 접수 / 면접 / 불합격 / 합격 (드롭다운) | ✅ 완료 |
| 일괄 작업 | 선택된 이력서 일괄 상태 변경/삭제 | ✅ 완료 |
...
| 다크 모드 | 라이트/다크 모드 토글 | ✅ 완료 |
| 페이지네이션 | 이력서 목록 페이지네이션 | ✅ 완료 |
...
|--------|------|------|
| id | UUID | 기본 키 |
| job_posting_id | TEXT | 공고번호 (고유, 중복 체크, UNIQUE) |
| job_posting_title | TEXT | 공고명 |
| job_detail | JSONB | 공고 상세 정보 (JSON) |
...
| 중복 체크 (이력서) | jobkorea_resume_id (Pass_R_No) 기준 |
| 처리 방식 | 순차 처리 (500ms 딜레이) |
| 이력서 페이지 크기 | 100개씩 보기 |
...
| AI 검토 입력 | `job_detail_md` + `resumes.md_url`의 Markdown 전문 |
